...
### ### ### ### ### ### ### ### ### ### ### ### ### #
### ### ### ### ### ### ### ### ### ### ### ### ### #
###           r-script 2_id_date_panels_set2.R    ###
###                Replication file               ###        
...
# exemption) throughout the study period of six months.
...
input_folk = "W:/ASMA1/data/intermediate/folk_data_20" # folk_data_20XX.csv, XX in 11:20

# Intermediate outputs (also used as inputs):
#interim1 = "W:/ASMA1/data/intermediate/id_date_panels/interim_set2_id_dateAA_genderB.rds" # where AA in 11:19 and B in 1:2

# Outputs:
#output = "W:/ASMA1/data/intermediate/id_date_panels/analysis_set2_id_yearAA_genderB.rds" # where AA in 11:19 and B in 1:2
...
person_yr = person_yr[year - byear <= 20 & year - byear >= 16][, byear := NULL]
...
# For those born between January 1st and June 30th, we take person-year
# observations where the person is aged 16 to 18 at the end of year:

# For those born between July 1st and December 31st, we take person-year
# observations where the person is aged 17 to 19 at the end of year:

person_yr = person_yr[(bday_18_month %in% c(1:6) & age %in% c(16:18)) |
                        (bday_18_month %in% c(7:12) & age %in% c(17:19))]

# Each person should have three person-year observations:
ids = person_yr[, .N, by='id'][N==3, id]
...
# window (a bandwidth of six months).

ids = person_yr[, .(unique_copay = length(unique(copay))), 
                by='id'][unique_copay==1, id]
...
# of year at 16 or 17. Specifically, for those born between January 1st and 
# June 30th, we take person-year observation where the person is aged 16 
# at the end of year. For those born between July 1st and December 31st, we take
# person-year observation where the person is aged 17 at the end of year:

ids = person_yr[(municipality %in% c(49, 853, 858) & bday_18_month %in% c(1:6) & 
                   age==16) | (municipality %in% c(49, 853, 858) &
                                 bday_18_month %in% c(7:12) & age==17)]
...
for(i in 2011:2019) {
  ...
  for(j in 1:2) {
    ...
    path = paste(
      'W:/ASMA1/data/intermediate/id_date_panels/interim_set2_id_date',
      i_char, '_gender', j_char, '.rds', sep=''
    )
    print(path)
    
    # Extract:
    data = person_yr[year == i & gender == j][, ':=' (year=NULL, gender=NULL)]
