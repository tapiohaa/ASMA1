...
person_ids = lapply(years, function(year) {
  ...
  visits = data.table::fread(
    source, select=c('id', 'birth_month', 'birth_year', 'birth_day', 'year')
  )
  
  # Take unique person-year observations:
...
# Examine the issue:
doubles = unique(person_ids[, .(id, birth_date)])[, .N, by='id']
...
person_ids = person_ids[!(id %in% doubles[N>1, unique(id)])]
rm(doubles)
...
# Create an indicator for whether the individual had a primary care visit during
# the year then they turned 17 or 18:
persons.1 = person_ids[year == bday_18_year | year==(bday_18_year - 1), id]
person_ids[, visit.17.or.18 := as.integer(id %in% persons.1)]

# Keep only unique rows:
person_ids[, year := NULL]
person_ids = unique(person_ids)
person_ids[, .N, by='id'][, max(N)]
...
# Check the share of individuals with a birth day observed of those who turned 
# 18 during 2011-2019:
test = folk[age==18, .(id, gender, income_eq)]
test = merge(test, person_ids, by='id', all.x=TRUE)
100 * (1 - test[, mean(is.na(birth_date))])

# The mean and median income of those women who turned 17:
test = folk[age==17 & gender==2, .(id, gender, income_eq)]
test = merge(test, person_ids, by='id', all.x=TRUE)
test[is.na(visit.17.or.18), visit.17.or.18 := 0]
test[, .(mean = mean(income_eq), median = median(income_eq))]

# The mean and median income of those women who turned 17 by whether or
# not they had a primary care visit during year in which they turned 17 or 18:
test[, .(mean = mean(income_eq), median = median(income_eq)), 
     by='visit.17.or.18']

# The mean income by income group among women who turned 17 and who had a 
# primary care visit during year in which they turned 17 or 18:

test = folk[age==17 & gender==2, .(id, gender, income_eq, income_decile)]
test = merge(test, person_ids, by='id', all.x=TRUE)
test = test[visit.17.or.18 == 1]

test.all = test[income_decile %in% c(0:9), mean(income_eq)]
test.b20 = test[income_decile %in% c(0:1), mean(income_eq)]
test.b40 = test[income_decile %in% c(0:3), mean(income_eq)]
test.t50 = test[income_decile %in% c(5:9), mean(income_eq)]

c(test.all, test.b20, test.b40, test.t50)
