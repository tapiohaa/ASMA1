...
### ### ### ### ### ### ### ### ### ### ### ### ### #
### ### ### ### ### ### ### ### ### ### ### ### ### #
###         r-script 2_analysis_data_set1.R       ###
###                Replication file               ###        
...
# Content: Create datasets for analyses. These are the main analysis datasets.
rm(list=ls())
...
#panel = "W:/ASMA1/data/intermediate/id_date_panels/analysis_set1_id_yearAA_genderB.rds" # where AA in 11:19 and B in 1:2

# Outputs:
output_local_pol = "W:/ASMA1/data/cleaned/data1_local_pol.rds"
output_parametric = "W:/ASMA1/data/cleaned/data1_parametric.rds"
output_local_rand = "W:/ASMA1/data/cleaned/data1_local_rand.rds"
  ...
  data = lapply(1:2, function(j) {
    ...
    df = df[!is.na(income_decile_17), 
            .(gp_visits = sum(gp_visits),
              referrals = sum(refs),
              health_checks = sum(health_checks),
    ...
    df[, ':=' (gp_visits_ann = gp_visits * 365 / population,
               referrals_ann = referrals * 365 / population,
               health_checks_ann = health_checks * 365 / population)]
...
df[is.na(income_decile_17), sum(referrals)]
df[!is.na(income_decile_17), sum(referrals)]
...
# When years change, policy (determined at the  municipality level) can 
# change as it is measured only annually. We keep only those who have unique 
# values for policy throughout the whole window.

ids = df[, .(unique_policy = length(unique(policy))), by='id'
         ][unique_policy == 1, id]
...
data = lapply(bandwidths, function(bw) {
  ...
  dfs = lapply(donut_holes, function(dh) {
    ...
    data = data[, .(income_decile_17 = unique(income_decile_17),
                    ...
                    gp_visits = sum(gp_visits),
                    referrals = sum(refs)), by=c('id', 'post_d')
    ][, ':=' (bw = bw, donut_hole = dh)]
...
data[, ':=' (gp_visits_ann = gp_visits * (365 / (bw-donut_hole)),
             referrals_ann = referrals * (365 / (bw-donut_hole)))]
...
# The number of total referrals:
sum(data[bw==30 & donut_hole == 0]$referrals)
